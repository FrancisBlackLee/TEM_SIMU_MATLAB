function [zoneAxes, bases, convMat, cutoff] = BasesAdvisor(cellLengths,...
    cellAngles, uvw)
%BasesAdvisor.m provides under the preset rule the advised reoriented zone
%axes, lattice vectors (bases) and the corresponding conversion matrix of 
%the unit cell for any crystal system.
% Input:
...
crysSys = CrystalClassifier(cellLengths, cellAngles);
switch crysSys
    case 'cubic'
        [zoneAxes, bases, convMat, cutoff] = CubicBasesAdvisor(cellLengths,...
            cellAngles, uvw);
    case 'rhombohedral'
        [zoneAxes, bases, convMat, cutoff] = RhomboAxisBasesAdvisor(cellLengths,...
            cellAngles, uvw);
    case 'hexagonal'
        [zoneAxes, bases, convMat, cutoff] = HexAxisBasesAdvisor(cellLengths,...
            cellAngles, uvw);
    case 'tetragonal'
        [zoneAxes, bases, convMat, cutoff] = TetraBasesAdvisor(cellLengths,...
            cellAngles, uvw);
    case 'orthorhombic'
        [zoneAxes, bases, convMat, cutoff] = OrthoBasesAdvisor(cellLengths,...
            cellAngles, uvw);
    case 'monoclinic'
        [zoneAxes, bases, convMat] = NormalBasesAdvisor(cellLengths,...
            cellAngles, uvw);
        cutoff = true;
    case 'triclinic'
        [zoneAxes, bases, convMat] = NormalBasesAdvisor(cellLengths,...
        ...
        cutoff = true;
    otherwise
        error('BasesAdvisor failed to generate bases for the supercell.');
end
