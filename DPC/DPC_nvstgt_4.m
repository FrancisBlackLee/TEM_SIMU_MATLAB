% DPC_nvstgt_4.m retrieval using moment of inertia (MOI)
% Not expecting this would yield any more information than that using COM.
% A different method must be developed, and it will not be announced before
...
COMstren = sqrt(COMx.^2 + COMy.^2);
% Show COM:
figure;
subplot(1, 2, 1);
imagesc(Scan_X, Scan_Y, COMstren);
colormap('gray'); axis square;
title('COMstren');
subplot(1, 2, 2);
plot(Scan_X, 2 * pi * COMstren(Scan_Ny / 2 + 1, : )/ InterCoeff);
title('2\pi COM / \sigma');
% Show dCOM:
figure;
subplot(1, 2, 1);
imagesc(x, y, dCOM);
