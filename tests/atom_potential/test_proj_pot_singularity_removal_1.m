% test_proj_pot_singularity_removal_1.m
clc;
clear;
close all;
%% main:
...
figure;
for atomType = 40 : 103
    folder = strcat('tests\atom_potential\threshold_data\Z=',...
        num2str(atomType));
    mkDirStat = mkdir(folder);
    
    for dx = 0.01 : 0.01 : 0.2
        Nx = 256;
        Lx = dx * Nx;
        x = -Lx / 2 : dx : Lx / 2 - dx;
        ...
        for frac = 0.1 : 0.1 : 1
            ...
            for atomIdx = 1 : atomNum
                tmpProjPot = ProjPotTestKernel(Lx, Lx, Nx, Nx, atomType,...
                    rndX(atomIdx), atomY, threshold);
            ...
            title(['dx = ', num2str(dx, '%.2f'),...
                ', frac = ', num2str(frac, '%.2f')]);
            drawnow;
            filename = strcat('dx=', num2str(dx, '%.2f'), '_frac=',...
                num2str(frac, '%.2f'), '.jpg');
